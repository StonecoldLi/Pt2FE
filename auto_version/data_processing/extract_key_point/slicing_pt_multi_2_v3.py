...
def slice_save_and_add_gap_to_point_cloud(pcd, direction, slice_distance, gap_distance, output_folder, file_prefix):
    ...
    os.makedirs(info_folder, exist_ok=True)  # 确保信息文件夹被创建
    ...
    while current_slice_start < max_projection:
        ...
        if len(slice_points) > 0:
            ...
            slice_filename = f"{file_prefix}_slice_with_gap_{slice_index:03d}.pcd"
            o3d.io.write_point_cloud(os.path.join(output_folder, slice_filename), slice_pcd)
            ...
            slice_csv_filename = os.path.join(info_folder, f"{file_prefix}_slice_with_gap_{slice_index:03d}.csv")
...
# 示例用法
#input_folder = './data_slicing/plane3_org'
#output_folder = './data_slicing/plane3_split'
